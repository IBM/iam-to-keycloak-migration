...
cp4iNamespace="integration"
commonServicesNamespace="ibm-common-services"
cp4iInstanceId="integration-fs34sd"
...
keycloakApiEndpoint="$keycloakUrl/admin/realms/$keycloakRealm" # should be adminApiEndpoint
...
function getTeamGroups() {
    # echo ""
    # echo "Getting filtered lists of admin, editor and viewer users"
    groupAdmins=$(echo $1 | jq -r ".usergroups | map(select(.roles[].id == \"crn:v1:icp:private:iam::::role:Administrator\")) | map(.name)") # Need to consider CloudPak admin etc
